...
io.on("connection", (socket) => {
    ...
    try {
      ...
      if (io.sockets.adapter.rooms.has(lobbyId)) {
        ...
        if (io.sockets.adapter.rooms.get(lobbyId).size < 10) {
          const existingPlayers = await findUsersByGame(lobbyId);
          const newPlayer = await createUser(username, null, lobbyId);
          // send back entry permission to join room
          socket.emit("entry-permission", {
            lobbyId,
            existingPlayers,
            newPlayer,
          });
  ...
  socket.on("host-load-game", async ({ lobbyId }) => {
    try {
      ...
      io.to(lobbyId.toString()).emit("host-finished-loading", {
        lobby,
        players,
        questions,
      });
  ...
  socket.on("upload-score", async ({ player, score, rounds }) => {
    try {
      ...
      await updateUser(player.id, scorePercentage);

      const loadPromise = new Promise(async (resolve, reject) => {
        setTimeout(async () => {
          const playersInLobby = await findUsersByGame(lobbyId);
  ...
  socket.on("leave-lobby", async ({ lobbyId, player, isHost }) => {
    try {
      ...
      if (io.sockets.adapter.rooms.get(lobbyId.toString()).size === 1) {
        const deleteMsg = deleteLobby(lobbyId);
      ...
      if (isHost) {
        try {
          const players = await findUsersByGame(lobbyId.toString());
