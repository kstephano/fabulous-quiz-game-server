...
class Lobby {
  ...
  static findByID(id) {
    return new Promise(async (resolve, reject) => {
      try {
        let lobbyData = await db.query("SELECT * FROM lobbies WHERE id = $1;", [
          id,
        ]);
  ...
  static findByCategory(category) {
    return new Promise(async (resolve, reject) => {
      try {
        let gamesData = await db.query(
          `SELECT * FROM lobbies WHERE category = $1;`,
          [category]
        );
        const games = gamesData.rows.map((g) => new Lobby(g));
        resolve(games);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let gameData = await db.query(
          "INSERT INTO lobbies (category, rounds, difficulty, roundLimit) VALUES ($1, $2, $3, $4 ) RETURNING *;",
          [category, rounds, difficulty, roundLimit]
        );
