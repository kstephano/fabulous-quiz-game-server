...
class User {
  ...
  static getScoreList(lobby_id) {
    return new Promise(async (resolve, reject) => {
      try {
        let usersData = await db.query(
          `SELECT * FROM users WHERE lobby_id = $1 ORDER BY SCORE DESC;`,
          [lobby_id]
        );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let userData = await db.query(
          "INSERT INTO users (username, score, lobby_id) VALUES ($1, $2, $3) RETURNING *;",
          [username, score, lobby_id]
        );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let updatedUserData = await db.query(
          "UPDATE users SET score = $2 WHERE id = $1 RETURNING *;",
          [id, score]
        );
